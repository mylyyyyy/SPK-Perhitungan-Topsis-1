...
$tampil = $koneksi->query("SELECT b.nama_alternatif,c.nama_kriteria,a.nilai,c.bobot
      ...
      ORDER BY b.id_alternatif, c.id_kriteria"); // Mengurutkan berdasarkan id_alternatif, id_kriteria
  ...
  <div class="container">
    ...
    <div class="row">
      <div class="col-lg-8 col-lg-offset-2">
        <div class="panel panel-default">
          <div class="panel-heading">
            Matriks Ternormalisasi (r<sub>ij</sub>)
          </div>
          <div class="panel-body">
            <table class="table table-striped table-bordered table-hover">
              <thead>
              ...
              <tbody>
                ...
                foreach ($data as $nama => $krit) {
                  echo "<tr>
                      ...
                      <td>{$nama}</td>";
                  foreach ($kriteria as $k) {
                    echo "<td align='center'>" . round(($krit[$k] / sqrt($nilai_kuadrat[$k])), 5) . "</td>";
                  }
                  echo
                    "</tr>\n";
                }
    ...
    <div class="row">
      <div class="col-lg-8 col-lg-offset-2">
        <div class="panel panel-default">
          <div class="panel-heading">
            Matriks Keputusan Terbobot (y<sub>ij</sub>)
          </div>
          <div class="panel-body">
            <table class="table table-striped table-bordered table-hover">
              <thead>
              ...
              <tbody>
                ...
                foreach ($data as $nama => $krit) {
                  echo "<tr>
                      ...
                      <td>{$nama}</td>";
                  foreach ($kriteria as $k) {
                    $y[$k][$i - 1] = round(($krit[$k] / sqrt($nilai_kuadrat[$k])), 5) * $bobot[$k];
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tbody>
                <tr>
                  <?php
                  $yplus = array();
                  if($n = 2 || $n = 3|| $n = 4|| $n = 6|| $n = 7|| $n = 8|| $n = 9){
                  foreach ($kriteria as $k) {
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tbody>
                <tr>
                  <?php
                  $ymin = array();
                  if($k = 2 || $k = 3|| $k = 4|| $k = 6|| $k = 7|| $k = 8|| $k = 9){
                  foreach ($kriteria as $k) {
                    $ymin[$k] = [$k] ? min($y[$k]) : max($y[$k]);
                    echo "<th>{$ymin[$k]}</th>";
                  }}
                  else{ foreach ($kriteria as $k) {
      ...
      <div class="panel-body">
        <table class="table table-striped table-bordered table-hover">
          ...
          <tbody>
            ...
            // Mengurutkan nilai V secara menurun
            arsort($V);
            $rank = 1;
            foreach ($V as $index => $vi) {
                $rowIndex = $index + 1;
                echo "<script>var rankCells = document.getElementsByClassName('rank');
